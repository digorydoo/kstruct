...
class KstructBuilder private constructor() {
    ...
    class ListBuilder internal constructor() {
        ...
        fun addMap(lambda: KstructBuilder.() -> Unit) {
            ...
            builder.lambda()
            list.add(KstructNode(KstructMap(builder.children, builder.attributes)))
        }

        fun addList(lambda: ListBuilder.() -> Unit) {
            val builder = ListBuilder()
            builder.lambda()
            list.add(KstructNode(KstructList(builder.list)))
        }
    }

    fun set(key: String, value: Boolean?) {
        val wrapper = value?.let { KstructBoolean(value) } ?: KstructNull()
        children[key] = KstructNode(wrapper)
    }
    ...
    fun setMap(key: String, lambda: KstructBuilder.() -> Unit) {
        val builder = KstructBuilder()
        builder.lambda()
        children[key] = KstructNode(KstructMap(builder.children, builder.attributes))
    }

    fun setList(key: String, lambda: ListBuilder.() -> Unit) {
        val builder = ListBuilder()
        builder.lambda()
        children[key] = KstructNode(KstructList(builder.list))
    }
    ...
    companion object {
        fun build(lambda: KstructBuilder.() -> Unit): KstructNode {
            val builder = KstructBuilder()
            builder.lambda()
            return KstructNode(KstructMap(builder.children, builder.attributes))
        }
    }
}
